...
def log_action(username, action, details=''):
    try:
        conn = get_db()
        conn.execute("INSERT INTO staff_log (username, action, details) VALUES (?,?,?)",
...
def parse_pdf(pdf_path):
    ...
    try:
        ...
        # Invoice & order date from table row
        ...
        for i, line in enumerate(lines):
            if 'customer address' in line.lower():
                for j in range(i+1, min(i+5, len(lines))):
                    ...
                    if candidate and not any(x in candidate.lower() for x in ['shadowfax','delhivery','ekart','cod','pickup','valmo']):
        ...
        # Qty
        qty_m = re.search(r'SKU.*?Free Size\s+(\d+)', text, re.DOTALL)
        if qty_m:
            data['qty'] = qty_m.group(1)

        # Lookup product image from DB
        if data['sku']:
            conn = get_db()
...
def login():
    if request.method == 'POST':
        ...
        if user and user['password'] == p:
            ...
            log_action(u, 'LOGIN', f"IP: {request.remote_addr}")
...
def get_summary():
    ...
    returns = conn.execute("SELECT COUNT(*) FROM orders WHERE status IN ('Return','Return Requested','Return Picked')").fetchone()[0]
    ...
    # Recent batches
    recent = conn.execute("SELECT batch, COUNT(*) as cnt FROM orders WHERE batch!='' GROUP BY batch ORDER BY batch DESC LIMIT 10").fetchall()
    recent_batches = [{'batch': r[0], 'count': r[1]} for r in recent]
    ...
    return jsonify({
        ...
        'rto': rto, 'shipped': shipped, 'returns': returns,
        'today': today_count,
...
def get_orders():
    ...
    params = []
    if search:
        where.append("(awb LIKE ? OR order_id LIKE ? OR customer LIKE ? OR sku LIKE ? OR invoice_no LIKE ?)")
        s = f'%{search}%'
        params += [s, s, s, s, s]
    if status:
        where.append("status=?")
    ...
    rows = conn.execute(
        f"SELECT * FROM orders {where_clause} ORDER BY id DESC LIMIT ? OFFSET ?",
...
def update_status():
    ...
    if not awb or not status:
        return jsonify({'error': 'AWB and status required'}), 400
    conn = get_db()
...
def bulk_status():
    ...
    awbs = data.get('awbs', [])
    status = data.get('status', '').strip()
    if not awbs or not status:
        return jsonify({'error': 'AWBs and status required'}), 400
    conn = get_db()
    for awb in awbs:
        conn.execute("UPDATE orders SET status=?, updated_at=datetime('now','localtime') WHERE awb=?", (status, awb))
    conn.commit()
    conn.close()
    log_action(session.get('user','?'), 'BULK_STATUS_UPDATE', f"{len(awbs)} orders → {status}")
        ...
        try:
            conn.execute('''INSERT INTO orders 
                ...
                (o.get('order_date',''), o.get('customer',''), o.get('address',''),
                 ...
                 o.get('payment','COD'), o.get('batch',''), o.get('entity','SAI Enterprises'),
...
def scan_return():
    ...
    for photo in photos:
        if photo and photo.filename:
            ...
            save_path = os.path.join(IMAGES_FOLDER, fn)
    ...
    updated = False
    if order:
        conn.execute('''UPDATE orders SET status='RTO Received', return_remark=?, 
                        photo=?, updated_at=datetime('now','localtime') WHERE awb=?''',
    ...
    conn.commit()
    conn.close()
    log_action(session.get('user','?'), 'RETURN_SCAN', f"AWB:{awb} Condition:{condition}")
...
def get_returns():
    ...
    rows = conn.execute('''SELECT awb, order_id, customer, sku, courier, platform, batch, 
                           ...
                           WHERE status IN ('RTO','RTO Initiated','RTO Received','Return','Return Requested','Return Picked','Wrong Return')
                           ORDER BY updated_at DESC LIMIT 1000''').fetchall()
    conn.close()
...
def get_finance():
    ...
    rows = conn.execute('''SELECT batch, platform, courier,
                           ...
                           SUM(settlement - cost) as total_pnl,
...
def get_fraud():
    conn = get_db()
    # Suspicious: same address multiple RTOs
    rows = conn.execute('''SELECT address, pin, COUNT(*) as rto_count,
                           ...
                           AND address != '' AND pin != ''
                           GROUP BY pin HAVING rto_count > 2
...
def get_batches():
    conn = get_db()
    rows = conn.execute('''SELECT batch,
                           COUNT(*) as total,
                           COUNT(CASE WHEN status='Delivered' THEN 1 END) as delivered,
                           COUNT(CASE WHEN status IN ('RTO','RTO Initiated','RTO Received') THEN 1 END) as rto,
                           COUNT(CASE WHEN status='Pending' THEN 1 END) as pending,
                           COUNT(CASE WHEN status='Shipped' THEN 1 END) as shipped,
                           COUNT(CASE WHEN platform='Meesho' THEN 1 END) as meesho,
                           COUNT(CASE WHEN platform='Flipkart' THEN 1 END) as flipkart,
                           COUNT(CASE WHEN platform='Amazon' THEN 1 END) as amazon
                           FROM orders WHERE batch!=''
                           GROUP BY batch ORDER BY batch DESC''').fetchall()
    conn.close()
...
def export_excel():
    ...
    return send_file(output, as_attachment=True,
                     download_name=f'SAI_Enterprises_Export_{datetime.now().strftime("%d%b%Y")}.xlsx',
